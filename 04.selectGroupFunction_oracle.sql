...
-- mysql : length() - byte 수 검색 / oracle : 
select ename, length(ename)	from emp;

/* mysql과 차이점
 * 	- mysql은 table이 불필요한 검색시 from절 자체를 미적용
 * 	- SELECT 2+3; 5 
 * 	oracle의 경우
 * 	- SELECT 2+3 FROM dual;
 * 	- dual이라는 잉여 테이블을 사용해야함.
 */
...
select deptno, count(comm) from emp; -- 오류. 12개의 deptno데이터와 1개의 count(comm)데이터
...
-- 오라클 실행 순서 : from -> group by -> having -> select -> order by
-- step01 - 오류 : 평균급여 식별자 문제 발생
select deptno, count(*) 직원수, count(comm) 커미션받는수, round(avg(sal)) 평균급여
from emp e
group by deptno
having 평균급여 >= 2000;

-- step01 - 해결책 : having avg(sal) >= 2000;
select deptno, count(*) 직원수, count(comm) 커미션받는수, round(avg(sal)) 평균급여
from emp e
group by deptno
having avg(sal) >= 2000;

-- step02 - 오류 : d 식별자 문제 발생
select deptno d, count(*) 직원수, count(comm) 커미션받는수, round(avg(sal)) 평균급여
from emp e
group by deptno
having 평균급여 >= 2000;

-- step02 - 해결책 : group by deptno 사용
select deptno d, count(*) 직원수, count(comm) 커미션받는수, round(avg(sal)) 평균급여
from emp e
group by deptno
having avg(sal) >= 2000;
