...
*/   
select ename, empno, sal from emp where deptno = 10;


/**2.
사원번호가 7369인 사람 중 이름, 입사일, 부서 번호를 출력하라.
*/
select ename, empno, deptno from emp where deptno = 7369;
...
/**4.
입사일이 83/01//12인 사원의 이름, 부서번호, 월급을 출력하라. -- 83년도 사원이 없음.
*/
select * from emp;
...
*/
select * from emp join dept on emp.deptno = dept.deptno where job != 'MANAGER';
-- 중복 데이터 deptno 제거 필요
...
*/
select * from emp join dept on emp.deptno = dept.deptno where emp.deptno >= 20;
...
*/
select * from emp join dept on emp.deptno = dept.deptno where emp.ename like 'K%';
...
*/
select * from emp inner join dept on emp.deptno = dept.deptno where emp.empno not between 7654 and 7782;
...
/**20.
커미션이 null인 사원의 정보를 출력하라. = 과 is의 차이가 뭔지..?
*/
...
*/
select emp.ename, dept.deptno, emp.sal from emp 
...
/**24.
부서번호가 30인 사람중 사원번호 정렬하라. -- 오름차순 내림차순 정렬이 안 적혀있음.
*/
...
/**25.
급여가 많은 순으로 정렬하라. - 급여가 많은 순으로 정렬하는데 어떤 데이터를 정렬하는건지
*/
