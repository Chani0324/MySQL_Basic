...
-- 3. emp의 구조 검색
-- oracle 명령어로 실행 가능. 단, DBeaver에선 desc를 oracle db 접속시엔 문법 오류 발생.
-- sqlplus라는 tool 상에서는 정상 실행
-- desc emp;
...
-- oracle에서는 Mysql과 다르게 대소문자 구분 명확히 함
...
select ename, comm from emp where nvl(comm, 0) not in (300, 500 ,1400);
...
select ename 사원이름, empno 사원번호, nvl(comm, 0) 상여금, sal*12 + nvl(comm, 0) 연봉
from emp 
